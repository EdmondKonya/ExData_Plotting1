...
#Using the with statement to enable adding additional lines to the plot
with(energy_data, 
     {
     #Initializing the plot with the first Sub_metering data
     plot(x = datetime, y = Sub_metering_1, type = "l", main = "", xlab = "", ylab = "Energy sub metering")
     
     #Adding the additional Sub_metering data with different colours
     lines(x = datetime, y = Sub_metering_2, col="red", type="l")
     lines(x = datetime, y = Sub_metering_3, col="blue", type="l")
     }
    )
  
  #Defining the legend for each Sub_metering column in the dataset
  legend("topright", 
